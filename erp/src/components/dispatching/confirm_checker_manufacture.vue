<template>
	<el-dialog :before-close="close" title="审核生产制定单" :visible.sync="dialog_visible">
...
<script>
	...
	export default{
		name:'confirm_checker_manufacture',
		model:{
			prop:'dialog_visible',
			event:'update'
		},
		...
		data(){
			...
			pass(){
				...
				this.$axios.post('/api/manufacture/pass',manufacture)
					.then(response=>{
						if(response.statusCord == 200){
							this.$message.success('产品制定单审核通过成功!')
							this.update_manufacture()
							this.close()
						}else{
				...
				this.$axios.post('/api/manufacture/reject',manufacture)
					.then(response=>{
						if(response.statusCord == 200){
							this.$message.success('产品制定单已驳回!')
							this.update_manufacture()
							this.close()
						}else{
							this.$message.error(response.message)
			...
			product_id(){
				...
				let material_price_sum = this.manufacture.material_cost_price_sum
				let labour_cost_price_sum=this.manufacture.labour_cost_price_sum
				let total_price=material_price_sum+labour_cost_price_sum
