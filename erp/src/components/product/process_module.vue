...
<script>
	export default {
		...
		data() {
			...
			put(data){
				let index = this.uncheckedProcess.findIndex(item=>{
					return item.id==data.id
				})
				this.$emit('put_process_detail',this.uncheckedProcess[index])
				this.uncheckedProcess.splice(index,1)
			...
			put_and_close(data){
				...
				let data=JSON.parse(JSON.stringify(this.process))
				return data;
			}
		},
		created() {
			...
			process_dialog_visible(newVal){
				if(newVal){
					this.getProcess()
					this.uncheckedProcess=this.uncheckedProcess1()
				}
			}
		},computed:{
