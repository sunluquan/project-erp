	...
	<div id="inventoryRegistration">
		...
		<el-row type="flex" class="row-bg" justify="space-around">
		   ...
		   :default-sort = "{prop: 'sort', order: 'ascending'}" @sort-change="sortChange">
			  ...
			  <el-table-column label="登记" width="120px">
			  	<template slot-scope="scope">
			  		<el-button type="text" @click="registration(scope.row.gatherId)">登记</el-button>
			  	</template>
			  </el-table-column>
		    </el-table>
	</div>
...
<script>
	export default {
	  ...
	  methods: {
	  	 		...
	  	 		selectProList(){			
	  	 			this.$axios.post('/api/gather/gatherRegistrationList').then(response=>{
	  	 				this.registrationPage=response.data;
						
	  	 				})
	  	 		},				   
				registration(gatherId){					
					this.$router.push({path: '/index/inventoryRegistration.html/inventoryRegistrationSheet/'+gatherId})
				},	  	 		
