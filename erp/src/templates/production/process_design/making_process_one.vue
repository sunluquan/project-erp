<template>
	<el-dialog width="70%" :before-close="close" title="产品生产工序设计" :visible.sync="dialog_visible">
		...
		<div id="columns">
			<el-form label-position="left" inline class="demo-table-expand">
				...
				<el-form-item label="产品名称: ">
					<span>{{productProcessDesign.product_name}}</span>
				</el-form-item>
			</el-form>
		</div>
		<div id="columns">
		...
		<el-table :summary-method="getSummaries" show-summary style="text-align: center;width: 100%" :border="true" :data="productProcessDesign.processDetails"
		 ref="process_design" :highlight-current-row="true" :default-sort="defaultSort">
			<template slot="empty">
				暂无工序,
			...
			<el-table-column label="工序执行顺序" prop="details_number" sortable>
				<template slot-scope="scope">
					...
					<number-order 
					:max="productProcessDesign.processDetails.length"
					 :min="1" 
					 v-model="scope.row.details_number"
					 @anew_order="anewOrder">
					 </number-order>
				</template>
			</el-table-column>
...
<script>
	import number_order from '@/components/process/number_order.vue'
	...
	export default {
		...
		data() {
			return {
				...
				timeUnits: [], //时间单位数组
				selected: [],
				defaultSort:{prop: 'details_number', order: 'ascending'}
			}
		},
		methods: {
			anewOrder(){
				this.$refs.process_design.sort(this.defaultSort.prop,this.defaultSort.order)
			},
			update(){
				this.$emit('update')
			},
			...
			submitProcessOneDesign() {
				...
				if(processDetails.length>0){
					this.$axios.post('/api/processDesign/insert',this.productProcessDesign)
						.then(response=>{
							if(response.statusCord==200){
								this.$message.success('新增产品工序设计成功!')
								this.update()
								this.close();
							}else{
					...
					this.$message.error('请至少选择一道设计工序');
				}
			},
			close() {
			...
			init() {
				this.getTimeUnit();
				let data = this.$route.params[Object.keys(this.$route.params)[0]]
				data = JSON.parse(data)
				for (let lt in data) {
					this.productProcessDesign[lt] = data[lt]
				}
			},
