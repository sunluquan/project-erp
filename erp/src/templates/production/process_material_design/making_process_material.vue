<template>
	<div id="making_process_material">
		<router-view @update="getAccordMakingProcessMaterial"></router-view>
		<!-- 
		...
		<el-table @row-dblclick="viewProcessDetail"
		 ...
		 :default-sort="{prop: 'product_id', order: 'descending'}" >
			...
			<el-table-column align="right" width="100px">
				...
				<template slot-scope="scope">
					<el-button type="primary" size="small" title="制定工序单" @click="makingProcessMaterial(scope.row.id)">制定工序物料</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
	...
	export default{
		name:'making_process_material',//制定工序物料
		data(){
			return{
				processDesigns:[],
				keyWord:'',
			...
			getAccordMakingProcessMaterial(){//获取符合制定工序物料条件的产品集合
				this.$axios.get('/api/processDesign/accordMakingProcessMaterial')
					.then(repsonse=>{
						this.processDesigns=repsonse.data
					})
			},makingProcessMaterial(id){//制定工序物料
				//makingProcess.html/design/id
				let fullPath=this.$route.fullPath
				this.$router.push({path:`${fullPath}/making/${id}`})
			...
			this.getAccordMakingProcessMaterial()
		},computed:{
			search() {
