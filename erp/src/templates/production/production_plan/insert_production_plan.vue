<template>
	<el-dialog width="70%" :before-close="close" title="产品生产计划设计" :visible.sync="dialog_visible">
		...
		<br />
		<el-table style="text-align: center;width: 100%" :border="true" :summary-method="getSummaries" show-summary :data="productionPlanDetails"
	...
	export default{
		...
		data(){
			...
			submitProductionPlan(){
				let products=this.productionPlanDetails
				if(products.length>0){
					...
					this.$axios.post('/api/productionPlan/insert',this.productionPlan)
						.then(response=>{
							if(response.statusCord == 200){
								this.$message.success('新增生产计划单成功!')
								this.update()
								this.close()
							}else{
